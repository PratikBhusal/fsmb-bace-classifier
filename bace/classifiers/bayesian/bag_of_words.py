...
def read_data(filename: Text) -> Tuple[List[Text], List[Text], List[Text]]:
    ...
    Tuple[List[Text], List[Text], List[Text]]
        returns the list of the filenames, class labels, and tokens in three
        separate lists
    """
    dataset = pd.read_csv(filename)
    return (
        dataset['filename'].tolist(),
...
def get_classifier(
    train_labels: List[Text], train_data: List[Text], num_features: int = 200
) -> GaussianNB:
    """
    trains the bag of words model based on the training labels and the training
    data, with a given number of features
    ...
    classifier = GaussianNB()
    classifier.fit(
        CountVectorizer(max_features=num_features)
...
def predict(
    classifier: GaussianNB, test_data: List[Text], num: int = 200
) -> Sequence[Sequence[float]]:
    """
    predicts the class label probabilities for the test set based on the test
    tokens and given number of features
    ...
    return classifier.predict_proba(
        CountVectorizer(max_features=num).fit_transform(test_data).toarray()
    )
...
def show_metrics(
    test_labels: List[Text], class_prediction: Sequence[Text]
) -> None:
    """
    prints the statistics for the prediction results
    confusion_matrix:       n x n array
                            number value on the diagonal: (0,0) ... (n,n)
...
def run_bagofwords(args: Namespace) -> None:
    """
    ...
    _, train_labels, train_tokens = read_data(args.training_file)
    ...
    if args.slice:
        ...
        show_metrics(
            test_labels, predict_single(clf, test_tokens, num=args.num_features)
        )
    else:
...
def construct_parser_bow(subparser: ArgumentParser) -> None:
    """
...
class BagOfWords(Classifier):
    ...
    def get_classifier(train_labels: List[Text], train_data:List[Text], num: int = 500):
        classifier = GaussianNB()
        classifier.fit(
            CountVectorizer(max_features = num).fit_transform(train_data).toarray(),
            train_labels
        )
        return classifier
